...
def runner():
    ...
    try:
        ...
        if train:
            ...
            # while total_number < 10:
                ...
                for t in range(args.episode_length):
                    ...
                    action = agent.get_action(observation, t, reward, done, train=True)
                    # print(action)
                ...
                print('Episode: {}'.format(episode),', Timestep: {}'.format(timestep),', Reward:  {:.2f}'.format(current_ep_reward),', Average Reward:  {:.2f}'.format(cumulative_score),', Time: {}'.format(t5))
                if episode % 10 == 0:
                    agent.learn()
                    agent.chkpt_save()
        ...
        import traceback; print("[RUNNER-FAIL]", type(_e).__name__, _e); traceback.print_exc()
        raise
    finally:
        pass
