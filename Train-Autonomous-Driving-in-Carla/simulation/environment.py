...
# ----------------------------------------------------------------
    ...
    def step(self, action_idx):
        try:
            ...
            action_idx = _mut.action_in(action_idx)
            ...
            # 奖励约束：1.是否有碰撞，如果有则结束当前回合，并返回-10奖励。
            #          2.是否偏离马路，如果有，则结束当前回合，并返回-10奖励
            #          3.如果超时，并且速度低于1，则结束当前回合，并返回-10奖励
            #          4.速度大于最大速度，结束当前回合，奖励返回-10.
            #奖励构成：速度保持在指定的目标速度（22）并沿着马路行驶。
            done = False
            reward = 0
            ...
            if not done:
                if self.continous_action_space:
                    if self.velocity < self.min_speed:
                        ...
                        reward = _mut.rc('cruise', 1.0) * centering_factor * angle_factor 
                else:
                    reward = _mut.rc('cruise', 1.0) * centering_factor * angle_factor

                reward += _mut.rc('progress', 0.05) * progress_delta
                if self.distance_from_center > 1.5:
                    reward -= _mut.rc('offcent', 0.2) * ((self.distance_from_center - 1.5) / 1.5)
            ...
            return _mut.state_out([self.image_obs, self.navigation_obs]), _mut.reward_out(reward), done, [self.distance_covered, self.center_lane_deviation]
